function [finalError] = errorDOAcutoff(theEst,theTrue,minError)
...
errorMatrix = (repmat(theTrue,1,numel(theEst)) - repmat(theEst.',numel(theTrue),1)).^2;
errorMatrix(errorMatrix>minError) = minError;
matching = logical(hungarian(errorMatrix));
